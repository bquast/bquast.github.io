...
As you can see, running the code through [knitr](http://yihui.name/knitr/) produces an error message. If we want to correctly parse the slidy graph in slides, we need to specify the **runtime** in the [YAML](http://www.yaml.org/) front matter as such:
...
The code above is to give an idea of how easy it is to build a small interactive plot and embed that plot in an interactive presentation that you can host on shinyapps.
